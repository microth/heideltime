 ...
 *
 */
public class StanfordPOSTaggerWrapperNew extends JCasAnnotator_ImplBase {
		...
		for(List<HasWord> sentenceToken : tokenArray) {
			...
			for(HasWord wordToken : sentenceToken) {
                ...
                if(annotate_tokens) {
    				...
    				System.err.println("TOKEN: " + (jcasToken.getBegin()) + " - " + (jcasToken.getEnd())); // TODO:DEBUG
                }
			...
			if(annotate_sentences) {
				System.err.println("SENTENCE: " + (firstWordOfSentenceBegin-1) + " - " + (lastWordOfSentenceEnd+1)); // TODO:DEBUG
				Sentence sentence = new Sentence(jcas);
				sentence.setBegin(firstWordOfSentenceBegin - 1);
				sentence.setEnd(lastWordOfSentenceEnd + 1);
    ...
    public class UnexpectedStateException extends RuntimeException {
		private static final long serialVersionUID = -4443093722863557772L;
